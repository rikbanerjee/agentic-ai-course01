# Agentic AI System Design – Course Prep

This markdown document is a structured prep curriculum designed to mirror the focus and style of the Maven course **“Building Agentic AI Applications with a Problem‑First Approach”**.[web:3][web:24]
...
Based on the public syllabus and associated materials, this prep focuses on building the same core capabilities as the course:[web:3][web:24]

- **Problem-first thinking**: framing business problems and designing AI systems around constraints (latency, cost, accuracy, risk) instead of starting from models or tools.[web:3][web:19]
- **Context engineering**: using decomposition, meta‑prompts, and structured prompting as a design tool, not a bag of tricks.[web:3][web:23]
- **Evaluation as backbone**: building product-level evals, LLM judges, and eval flywheels to iteratively improve systems.[web:3][web:19]
- **Agentic retrieval & RAG**: integrating retrieval, memory, and self‑reflection into agents rather than treating RAG as a bolt‑on feature.[web:3][web:24]
- **Multi-agent systems & MCP/A2A**: coordinating multiple agents and tools via explicit protocols and contracts, including MCP-style interfaces.[web:3][web:24]
- **End‑to‑end build & iteration**: repeatedly refining a multi‑agent agentic search system with RAG, MCP, and coordination patterns.[web:3]

### 0.2 Recommended background

The original course is accessible to builders who have basic terminal familiarity and are comfortable setting up tools and running scripts; it provides both low‑code and code‑heavy tracks.[web:3]

This prep assumes you:

- Have basic familiarity with Python or TypeScript (or are willing to learn enough to wire APIs and scripts).
- Can use the terminal for creating virtual environments, running scripts, and using Git.
- Have at least one LLM provider account (e.g., OpenAI, Anthropic, Gemini) and understand API keys at a basic level.[web:3]
...
- **Runtime**:
  - Python **3.10+**.
  - Environment manager: `uv`, `poetry`, or `pipenv` (or `venv` + `pip`) for isolation.
- **Version control**:
  - Git installed.
...
- **Vector store** (choose one):
  ...
  - Local: Chroma, LanceDB, or Postgres + `pgvector`.
- **Data processing**:
  - Libraries: `pandas`, `numpy`, `tiktoken` (or equivalent tokenizer), a PDF/HTML parser (e.g., `pypdf`, `beautifulsoup4`).

### 1.4 Observability & evaluation helpers

Not strictly required, but recommended to mirror course emphasis on evaluation and observability.[web:3][web:19]

- **Logging**:
  - Configure structured logs (JSON) with fields like `timestamp`, `user_id`, `request_id`, `prompt`, `model`, `latency_ms`, `tokens_in`, `tokens_out`.
- **Tracing / monitoring**:
  - Optional: Langfuse, Helicone, or a simple custom dashboard using Streamlit or notebooks.
- **Evaluation notebook**:
  - A dedicated notebook or script for running batch evals and visualizing metrics.

---

## 2. Domain & Data Preparation

To mirror the problem-first orientation, you should fix a domain and work on a realistic dataset throughout the prep.[web:3][web:24]
...
- `src/app.py` – entry point for experiments (CLI or API).
- `src/llm_client.py` – wrapper around your chosen LLM provider.
- `src/retrieval/` – RAG-related utilities.
- `configs/prompts/` – YAML/JSON prompt templates.
- `configs/evals/` – evaluation datasets.
- `notebooks/00_exploration.ipynb` – quick exploration of your corpus.
...
This module aligns you with the instructors’ vocabulary and conceptual framing using their free **Agentic AI Crash Course**.[web:24]

### 3.1 Objectives

- Understand the difference between vanilla GenAI and agentic systems.[web:24]
- Learn the **four types of agentic systems** and when to use each.[web:24]
- Get a high-level view of tools, RAG, and MCP from the course creators’ perspective.[web:24]

### 3.2 Materials

Work through the crash course sections:[web:24]
...
This mirrors the course’s first week: introduction, setup, and building foundational applications plus the first assignment.[web:3]

### 4.1 Objectives

- Explain why traditional software assumptions break down in AI-driven systems.[web:3][web:19]
...
- Why “shipping a prompt” is the start, not the end, of building a GenAI system.[web:3]

### 4.3 Labs

**Lab 1: Hello GenAI API**

- Implement a CLI or minimal HTTP API in `src/app.py` that:
  - Accepts a domain-specific user request (e.g., “Help me compare product X vs Y”).
  - Calls your LLM via `src/llm_client.py` with a **fixed base prompt**.
...
This mirrors the course’s emphasis on problem‑first, constraint‑aware design.[web:3][web:19]

---

## 5. Module 2 – Context Engineering & Enterprise Agents (Week 2)

Week 2 of the course focuses on **context engineering in 2026** and building agents suitable for enterprise environments.[web:3][web:23]

### 5.1 Objectives

- Use prompt decomposition and meta‑prompts to structure complex tasks.[web:23]
- Build an agent that uses context windows intelligently rather than “stuffing more text.”[web:3]

### 5.2 Concept work

- Study the “pillars” of GenAI systems described by the instructors: instructions, context, reasoning, and tools.[web:23][web:24]
...
- Understand how enterprise constraints (PII, compliance, access control) should appear in prompts and context policies.[web:3]
...
- Implement each step as a separate function + prompt; chain them in `src/workflows/decomposed_task.py`.
...
This mirrors the course’s focus on **context engineering for enterprise use cases**.[web:3]

---

## 6. Module 3 – Evaluation & Guardrails as Backbone (Week 3)

The course treats evaluation as the backbone of reliable agentic systems and emphasizes **product‑level evals** and **eval flywheels**.[web:3][web:19]
...
- Distinguish between:
  - **Model benchmarks** (e.g., standard LLM leaderboards) and **product evals** (task success in your app).[web:19]
...
- Implement `scripts/run_evals.py` that:
...
The course highlights integrating retrieval, memory, and **self‑reflective behavior** into agentic systems rather than treating RAG as an isolated feature.[web:3][web:24]
...
- Compare **traditional RAG** vs **agentic RAG**:[web:24]
...
- Understand design tradeoffs:
  ...
  - Latency vs accuracy vs cost.[web:4]
...
- Implement `src/retrieval/rag.py` with functions:
  - `retrieve(query) -> List[Document]`.
  - `answer_with_evidence(query, docs) -> str`, where the prompt forces citing sources.
...
The course explicitly covers **multi-agent systems**, coordination patterns, and protocols like **MCP/A2A** for more complex workflows.[web:3][web:24]
...
- Revisit the four agentic system types from the crash course and map them to multi-agent patterns.[web:24]
- Study tradeoffs in multi-agent systems:
  - Autonomy vs control.
  - Complexity vs reliability.
  - Latency cost of coordination.
- Understand MCP-style thinking:
  - Tools as typed interfaces with schemas for input/output and errors.[web:24]
...
- Define tool signatures as JSON schemas and validate arguments at runtime.
...
The Maven capstone centers on building an **end‑to‑end agentic AI solution**, specifically an agentic search system iterated through multiple versions and presented to a large audience.[web:3]
...
- Expose an `ask(query)` endpoint backed by your RAG implementation.
...
- Use logs and traces to:
  ...
  - Add new eval cases for those failures (your eval flywheel).[web:19]
...
The course runs in a cohort format with live sessions, async work, and capstone time; you can mirror this pacing.[web:3]
...
- 3–4 hours – “Live equivalent”:
  ...
  - Take active notes and immediately implement 1–2 ideas.[web:17][web:21]
...
This mirrors the course’s emphasis on **decision maps and tradeoffs** rather than checklist-style topic coverage.[web:3][web:19]
...
- Aim to complete **Modules 0–3** before the cohort starts so you can use live time on nuance and feedback rather than basic setup.[web:3][web:24]
...
- Run weekly review sessions where team members “present” their work, mirroring the course’s live project reviews.[web:3]

This document should give you a structured, repo‑friendly way to prepare for or mirror the **Building Agentic AI Applications with a Problem‑First Approach** course while aligning closely with its syllabus and philosophy.[web:3][web:24]